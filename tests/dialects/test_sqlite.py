...
class TestSQLite(Validator):
    ...
    def test_generated_columns(self):
        ...
        # Without PRIMARY KEY, identity must be dropped (bare AUTOINCREMENT is invalid).
        self.validate_all(
            "CREATE TABLE t (x INTEGER)",
            read={
                "": "CREATE TABLE t (x INT GENERATED BY DEFAULT AS IDENTITY)",
                "mysql": "CREATE TABLE t (x INT AUTO_INCREMENT)",
                "snowflake": "CREATE TABLE t (x INT AUTOINCREMENT)",
            },
            write={"sqlite": "CREATE TABLE t (x INTEGER)"},
        )
        self.validate_all(
            "CREATE TABLE t (x INTEGER)",
            read={
                "": "CREATE TABLE t (x INT GENERATED ALWAYS AS IDENTITY)",
                "postgres": "CREATE TABLE t (x INT GENERATED BY DEFAULT ON NULL AS IDENTITY)",
            },
            write={"sqlite": "CREATE TABLE t (x INTEGER)"},
        )
        self.validate_all(
